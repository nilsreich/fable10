...
- Vercel-Deploy → Demo-Build erzeugt self-contained `dist/` (Bundle + kopierte Assets, `theme.css` wird mitkopiert statt relativ referenziert); `vercel.json` mit buildCommand/outputDirectory → statisches Deployment ohne Framework-Preset, Dev-Server und Vercel nutzen denselben Build (keine Publishing-Pipeline im Sinne des Nicht-Ziels, nur ein statischer Demo-Build).
